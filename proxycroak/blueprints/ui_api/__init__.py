...
@blueprint.route("/proxies", methods=["POST"])
def proxies():
    META = {
        "title": "Proxies",
    ...
    # TODO: Include errors
    output = handle_pic_mode(parse_decklist(data["decks[0]"]), options)

    return render_template("pages/proxies.html", meta=META, rows=output)
