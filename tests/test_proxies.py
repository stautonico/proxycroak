...
def generate_lines():
    ...
    for rawcard in cards:
        # if sets[rawcard[23]]["id"] == "xyp":
        #     if rawcard[14] in ["XY39", "XY46", "XY68"]:
...
def test_cards(line):
    ...
    if "no image available" in response.get_data(as_text=True).lower():
        return
    ...
    # For some reason, this doesn't run the js, so we have to manually try to fallback to `small.webp`
    if img_response.status_code == 404:
        img_response = client.get(img.get("src").replace("large.webp", "small.webp"))
